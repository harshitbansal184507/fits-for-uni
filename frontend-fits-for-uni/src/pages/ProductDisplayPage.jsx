  ...
  const fetchProductDetails = async () => {
    try {
      ...
      if (responseData.success) {
        ...
        // Set the default selected size to the first size in the array
        setSelectedSize(responseData.data.sizes?.[0]?.size || 'freesize');
      }
    } catch (error) {
...
'>{data.description}</h2>
        ...
        {hasSizes && (
          <div className='my-4'>
            <label htmlFor='size-select' className='block text-sm text-gray-900 dark:text-gray-900'>
              Select Size
            </label>
            <select
              id='size-select'
